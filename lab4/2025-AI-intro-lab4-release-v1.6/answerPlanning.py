...
class RRT:
    ...
    def build_tree(self, start, goal):
        ...
        while True:
            rand_point = np.array(
                [np.random.uniform(self.x_min, self.x_max), np.random.uniform(self.y_min, self.y_max)]
            )
            ...
            if is_empty:
                ...
                if (
                    self.map.checkline(list(new_point), list(np.array(goal).astype(float)))[0] == False
                    or np.linalg.norm(new_point - goal) <= TARGET_THREHOLD
                ):
    ...
    def find_nearest_point(point, graph):
        ...
        for i in range(len(graph)):
            temp = np.linalg.norm(graph[i].pos - point)
    ...
    def connect_a_to_b(self, point_a, point_b):
        ...
        newpoint = point_a + STEP_DISTANCE * direction / norm
        if (
            self.map.checkline(list(point_a), list(newpoint)) == (False, None)
            and self.map.checkoccupy(newpoint) == False
        ):
            is_empty = True
        ### 你的代码 ###

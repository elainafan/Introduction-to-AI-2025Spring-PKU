...
class NaiveBayesModel:
    ...
    def __call__(self, text):
        ...
        p = [0, 0]
        for label in [0, 1]:
            p[label] = math.log(self.pos_neg_num[label] / self.V)
            for token in text:
                p[label] += math.log(
                    (self.token_num[label].get(token, 0) + alpha) / (self.pos_neg_num[label] + self.V * alpha)
                )
        return np.argmax(p)
...
class Embedding:
    def __init__(self):
        self.emb = dict()
        with (
            open("words.txt", encoding="utf-8") as f
        ):  # word.txt存储了每个token对应的feature向量，self.emb是一个存储了token-feature键值对的Dict()，可直接调用使用
...
class QAModel:
    ...
    def __call__(self, query):
        ...
        for tokens, sentence in self.document_list[best_document]["sentences"]:
            sentence_score.append(sum(self.idf(word) for word in query if word in tokens))
        best_sentence = np.argmax(sentence_score)
        return self.document_list[best_document]["sentences"][best_sentence][1]

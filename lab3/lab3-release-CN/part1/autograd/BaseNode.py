...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        k = X @ W_k
        v = X @ W_v
        scores = q @ k.transpose(0, 2, 1) / np.sqrt(X.shape[2])
        exp_scores = np.exp(scores - np.max(scores, axis=-1, keepdims=True))
        softmax_scores = exp_scores / np.sum(exp_scores, axis=-1, keepdims=True)
        attention_output = softmax_scores @ v
        output = attention_output + X
        self.cache.append((X, softmax_scores, q, k, v))
        return output
    ...
    def backcal(self, grad):
        ...
        return (
            grad
            + softmax_grad @ (W_q @ K.transpose(0, 2, 1)).transpose(0, 2, 1) / np.sqrt(self.dim)
            + softmax_grad.transpose(0, 2, 1) @ (Q @ W_k.T) / np.sqrt(self.dim)
            + softmax_score.transpose(0, 2, 1) @ grad @ W_v.T
        )
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        X = self.cache[-1]
        X_reshaped = X.reshape(-1, grad.shape[-1])
        grad_reshaped = grad.reshape(-1, grad.shape[-1])
        self.grad.append(np.dot(X_reshaped.T, grad_reshaped))
        self.grad.append(grad_reshaped.sum(axis=0))
        return (grad_reshaped.dot(self.params[0].T)).reshape(*X.shape) + grad
        # TODO: YOUR CODE HERE
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        temp_x = np.log(np.exp(X).sum(axis=self.dim, keepdims=True))
        ret = X - temp_x
        self.cache.append(np.exp(ret))  # 提前存exp值方便后面取
        return ret
        # TODO: YOUR CODE HERE

    def backcal(self, grad):
        ret = self.cache[-1]
        return grad - np.sum(grad, axis=self.dim, keepdims=True) * ret
        # TODO: YOUR CODE HERE
...
class NLLLoss(Node):
    ...
    def cal(self, X):
        y = self.y
        self.cache.append(X)
        return -np.sum(np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1))
